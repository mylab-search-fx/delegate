...
Для поиска `Delegate` получает в том числе и строку поиска. Эта строка может содержать несколько текстовых конструкций, используя которые `Delegate` дополняет запрос в `Elasticsearch` необходимыми условиями. 
...
* в противном случае используется сортировка, определённая `ES` по умолчанию - по релевантности.
