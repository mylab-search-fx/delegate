...
* в приоритете совпадение поля записи со всем текстом строки поиска;
...
### Запрос токена поиска

Запрос токена поиска перечисляет индексы, к которым должен иметь доступ клиент и ограничения по выборкам для этих индексов.

Структура запроса:

* `indexes`[] - настройки индексов:
  * `id` - литеральный идентификатор индекса;
  * `filters[]`(опционально) - фильтры, применяющиеся для текущего индекса;
    * `id` - идентификатор фильтра;
    * `args` (опционально) - аргументы фильтра:
      * `[any]` - именованные параметры - аргументы для фильтра.



`Wildcard` (`*`) - идентификатор индекса:
...
* фильтр по умолчанию из настроек индекса - поле `DefaultFilter`
...
* из настроек индекса - поле `DefaultSort` (обязателен, если указан)
* `default` (только если файл существует)
...
* если строка поиска состоит из нескольких литералов, добавляется сравнение поля записи со всей строкой поиска по полному совпадению или совпадению начала значения со строкой поиска. Такое совпадение обладает наибольшим весом релевантности среди других условий, равный количеству литералов в строке поиска + 1;
...
* по каждому литералу:
  ...
  * определяется, как условие по точному совпадению для полей типа [keyword](https://www.elastic.co/guide/en/elasticsearch/reference/current/keyword.html#keyword-field-type);

  * определяется, как объединение `should` условий по точному совпадению и по регулярному выражению [regexp](https://www.elastic.co/guide/en/elasticsearch/reference/current/query-dsl-regexp-query.html) для полей типа  [text](https://www.elastic.co/guide/en/elasticsearch/reference/current/text.html#text-field-type). Регулярное выражение формируется по правилу:
