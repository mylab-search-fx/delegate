...
Спецификация `API`: [![API specification](https://img.shields.io/badge/OAS3-v3%20(actual)-green)](https://app.swaggerhub.com/apis/ozzy/mylab-search-searcher/3) 
...
`Searcher` позволяет осуществлять поиск по разным сущностям. Разделение достигается за счёт абстракции `пространств имён`. `namespace` указывается в адресе при поиске. Для каждого пространства имён могут быть свои настройки и ресурсы (фильтры и сортировки).  
...
* `mylab:searcher:namespaces` - настройки сервиса `Searcher`:
